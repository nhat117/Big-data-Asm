...
twelvetable = os.getenv("twelvetable.table", "twelvedata")
...
TWELVE_TABLE = os.environ.get("TWELVE_TABLE") if os.environ.get("TWELVE_TABLE") else 'twelve'
...
def saveTwelveDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + twelvetable + " (datetime, macd, macd_hist, macd_signal ) \
                   VALUES (?, ?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['datetime'], val['macd'],
                           val['macd_hist'], val['macd_signal']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def getDF(source_table):
    ...
    if source_table not in (WEATHER_TABLE, TWITTER_TABLE, TWELVE_TABLE):
        return None
